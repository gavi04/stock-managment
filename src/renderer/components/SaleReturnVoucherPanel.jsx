...
export function SaleReturnVoucherPanel({ products, parties, busy, onSave, onUpdate }) {
    ...
    <div ref={rootRef} style={{ display: 'flex', flexDirection: 'column', gap: '16px', height: '100%' }}>
      <section className="panel" style={{ padding: '16px' }} onKeyDown={handleFormKeyNav}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px', alignItems: 'center' }}>
          <h2>{editingId ? `Editing Sales Return ${voucherNo}` : 'Sales Return Voucher'}</h2>
      ...
      <VoucherHistory type="sale_return" refreshToken={historyKey} title="Recent Sale Return Vouchers" partyLabel="Customer" onEdit={loadForEdit} />
    </div>
  );
}
