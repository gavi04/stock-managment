...
export function PurchaseVoucherPanel({ products, parties, busy, onSave }) {
  ...
  const handleRowChange = (index, field, value) => {
    ...
    if (field === 'product_id') {
      ...
      if (product) {
        newItems[index].base_rate = ''; // normally fetch from product or last purchase
        newItems[index].gst_rate = '18'; // default or fetch from product
      }
    }
  ...
  const handleKeyDown = (e, rowIndex, colIndex) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      // If we are on the last column (gst_rate), add new row and focus first col
      if (colIndex === 8) {
            ...
            <tbody>
              {items.map((row, i) => (
                <tr key={row.id}>
                  ...
                  <td><input value={row.net_rate} readOnly tabIndex={-1} style={{width: '100%', padding: '4px', background: '#eee'}} /></td>
                  <td><input value={row.taxable_value} readOnly tabIndex={-1} style={{width: '100%', padding: '4px', background: '#eee'}} /></td>
                  ...
                  <td><input value={row.amount} readOnly tabIndex={-1} style={{width: '100%', padding: '4px', background: '#eee'}} /></td>
      ...
      <section className="panel" style={{ padding: '16px' }}>
        <div style={{ display: 'flex', gap: '24px' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'flex-end', gap: '8px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', width: '200px', fontSize: '1.2em', color: '#2ecc71' }}>
